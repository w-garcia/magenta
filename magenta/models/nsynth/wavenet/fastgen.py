...
def sample_categorical(pmf):
    ...
    # Create rand val for each audio in batch
    ...
    for i in range(batch_size):
        idxs[i] = cdf[i].searchsorted(rand_vals[i]) # Returns an index to where the value landed?
        # print("Got category {}".format(idxs[i]))
        # raw_input(">")
    return idxs
...
def synthesize(encodings,
    ...
    print("encoding length: {}".format(encoding_length))
    ...
    with tf.Graph().as_default(), tf.Session(config=session_config) as sess:
        ...
        for sample_i in range(total_length):
            ...
            all = sess.run(
                ...
                feed_dict={net["X"]: audio, net["encoding"]: encodings[:, enc_i, :]}
            )
            pmf = all[0]
            ...
            print(np.shape(pmf))
            """
            Because raw audio is typically stored as a sequence of 16-bit integer values (one per timestep), 
            a softmax layer would need to output 65,536 probabilities per timestep to model all possible values. 
            To make this more tractable, we first apply a mu-law companding transformation (ITU-T, 1988) to the data, 
